...
discollect = cataList ( either (nil) (uncurry (++) . (dic >< id)) )
...
&
    |C + Seq(A) >< Seq((Exp c [a]))|
           \ar[d]^{|id + id >< (cataExp tar)|}
           \ar[l]_-{|inLTree|}
\\
     |(Seq(A)>< c)|
&
     |C + Seq(A) >< Seq((Seq((Seq(A) >< c))))|
           \ar[l]^-{|g|}
}
\end{eqnarray*}

\begin{code}
dic_rd s = lookup s . dic_exp

dic_in a b c = dic_imp(collect((++) (singl(split p1 p2 ((|->) a b))) (discollect (dic_exp (c)))))
\end{code}

\par{ Como não conseguimos usar hilomorfismos para definir estas funções ,dic rd e dic in, optamos por exportar 
o dicionário para listas e trabalhar sobre listas.}
...
\begin{eqnarray*}
\start
|lcbr(maisEsq Empty = Nothing)(maisEsq (Node (r,(e,d)) = if (isNothing e) then (Just r) else (maisEsq e))|
%
\just\equiv{Def-compara , Def-const , Fusão-const}
%
|lcbr(maisEsq . (const Empty) = (const Nothing))(maisEsq (Node (r,(e,d))) = compara (r,( maisEsq e,maisEsq d)))|
%
\just\equiv{Def-x , Natural-id}
%
|lcbr(maisEsq . (const Empty) = (const Nothing))(maisEsq (Node (r,(e,d))) = compara . (id >< (maisEsq >< maisEsq)) (r,(e,d)))|
%
\just\equiv{Igualdade extensional , Def-comp}
%
|lcbr(maisEsq . (const Empty) = (const Nothing))(maisEsq . Node = compara . (id >< (maisEsq >< maisEsq)))|
%
\just\equiv{Eq-+ , Fusão-+}
%
|maisEsq . either (const Empty) (Node) = either (const Nothing) (compara . (id >< (maisEsq >< maisEsq)))|
%
\just\equiv{Absorção-+}
%
|maisEsq . either (const Empty) (Node) = either (const Nothing) (compara)  . (id + (id >< (maisEsq >< maisEsq)))|
%
\just\equiv{Def. inBTree}
%
|maisEsq . in = either (const Nothing) (compara)  . (id + (id >< (maisEsq >< maisEsq)))|
%
\just\equiv{Universal-cata}
%
|maisEsq = cataBTree (either (const Nothing) (compara))|
\end{eqnarray*}
\begin{eqnarray*}
\xymatrix@@C=2cm{
    |BTree A|
...
&
     |1 + A >< (Maybe >< Maybe)|
      \ar[l]^-{\alt{\underline{Nothing}}{compara}}
}
\end{eqnarray*}

\begin{code}
compara :: ( a , ( Maybe a , Maybe a)) -> Maybe a
compara (r,(e,d))
              |(isNothing e) = Just r
              |otherwise = e

maisEsq = cataBTree g
  where g = either (const Nothing) compara

\end{code}

\par{ É fácil de perceber que o elemento mais à esquerda de uma árvore será o elemento mais à direita na árvore inversa.}

\begin{eqnarray*}
\start
|maisDir = maisEsq . invBTree|
%
\just\equiv{Def-invBtree}
%
|maisDir = maisEsq . (cataBTree ( either (const Empty) (Node . (id >< swap))))|
%
\just\equiv{Fusão-cata , devia ser simbolo de implica mas não sabemos faze-lo}
%
|maisDir . (id + id >< (maisEsq >< maisEsq)) = maisEsq .  either (const Empty) (Node . (id >< swap)) |
%
\just\equiv{ Absorção-+}
%
|maisDir . (id + id >< (maisEsq >< maisEsq)) = maisEsq .  (either (const Empty) (Node)) . (id + (id >< swap)) |
%
\just\equiv{Cancelamento-cata , pois (either (const Empty) (Node)) = in e maisEsq e um cataBTree}
%
|maisDir . (id + id >< (maisEsq >< maisEsq)) = either (const Nothing) (compara) .  (F maisEsq) . (id + (id >< swap)) |
%
\just\equiv{Propriedade gratis ( swap ) , Leibniz , Absorção-+}
%
|maisDir = either (const Nothing) (compara . (id >< swap)) |
\end{eqnarray*}
\begin{code}

maisDir = cataBTree g
  where g = ( either ( const Nothing ) (compara . (id >< swap)))
...
insOrd' x = cataBTree g 
  where g = split (either h1 h2) idd
        h1 () = emp x
...
ord:: Ord a =>  Either ()  (a,((Bool, BTree a),(Bool,BTree a))) -> Bool
ord = either (const True) testa



idd = either (const Empty) (Node . ( id >< ( p2 >< p2)))
...
\par{splay a l = cataBTree ( either (const Empty) (aux a l))}
\par{      where aux a [] (r,(e,d)) = Node (r,(e,d))}
\par{             aux a (h:t) (r,(e,d)) = if h then rrot ( aux a t (r,(e,d))) else  lrot ( aux a t (r,(e,d)))}
...
\begin{eqnarray*}
\start
|lcbr(extLTree (Dec a) = Leaf a)(extLTree (Query (s,(e,d)) = Fork ( extLTree e , extLTree d )|
%
\just\equiv{Def-proj}
%
|lcbr(extLTree (Dec a) = Leaf a)(extLTree (Query (s,(e,d)) = Fork . p2 ( s , ( extLTree e , extLTree d ))|
%
\just\equiv{Def-x , Natural-id}
%
|lcbr(extLTree (Dec a) = Leaf a)(extLTree (Query (s,(e,d))) = Fork . p2 . (id >< (extLTree >< extLTree) ) (s,(e,d)))|
%
\just\equiv{Igualdade extensional , Def-comp}
%
|lcbr(extLTree . Dec = Leaf)(extLTree . Query = Fork . p2 . (id >< (extLTree >< extLTree)))|
%
\just\equiv{Eq-+ , Fusão-+}
%
|extLTree . either(Dec) (Query) = either(Leaf) (Fork . p2 . (id >< (extLTree >< extLTree)))|
%
\just\equiv{Absorção-+}
%
|extLTree . either(Dec) (Query) = either(Leaf) (Fork . p2) . ( id + (id >< (extLTree >< extLTree)))|
%
\just\equiv{Def. inBdt}
%
|extLTree . in = either(Leaf) (Fork . p2) . ( id + (id >< (extLTree >< extLTree))) |
%
\just\equiv{Universal-cata}
%
|extLTree = cataBdt (either(Leaf) (Fork . p2))|
\end{eqnarray*}
...
&
     |A + S >< (LTree A >< LTree A)|
      \ar[l]^-{\alt{Leaf}{Fork\comp\p2}}
}
\end{eqnarray*}
\begin{code}
inBdt = either (Dec) (Query)
outBdt (Dec a) = i1 (a)
outBdt (Query(s,(e,d))) = i2 (s,(e,d))
\end{code}
...
|outBdt . inBdt = id |
%
\just\equiv{Def.inBdt , Reflexão-+}
%
|outBdt . either(Dec) (Query) = either(i1) (i2)|
%
\just\equiv{Fusão-+}
%
|either(outBdt . Dec) (outBdt . Query) = either(i1) (i2)|
%
\just\equiv{Eq-+}
%
|lcbr(outBdt . Dec = i1)(outBdt . Query = i2)|
%
\just\equiv{Igualidade extensional , Def-comp}
%
|lcbr(outBdt (Dec a) = i1 (a))(outBdt (Query (s,(e,d)) = i2 (s,(e,d)))|
\end{eqnarray*}
...
\par{Começamos por definir a função depth.}

\begin{eqnarray*}
\start
|lcbr(depth (Leaf a) = 1)(depth Fork(e,d) = 1 + max (depth e) (depth d))|
%
\just\equiv{Def-x , Def-const , Uncurry}
%
|lcbr(depth (Leaf a) = ((const 1) . a) )(depth Fork(e,d) = 1 + (uncurry max) . (( depth >< depth ) (e,d)))|
%
\just\equiv{Def-comp , Igualdade extensional , Def-succ}
%
|lcbr(depth . Leaf = (const 1))(depth . Fork = succ . (uncurry max) . ( depth >< depth ))|
%
\just\equiv{Eq-+ , Fusão-+}
%
|depth . either(Leaf) (Fork) = either(const 1) (succ . (uncurry max) . ( depth >< depth ))|
%
\just\equiv{Absorção-+}
%
|depth . either(Leaf) (Fork) = either(const 1) (succ . (uncurry max)) . ( id + (depth >< depth ))|
%
\just\equiv{Def. inLTree}
%
|depth . in = either(const 1) (succ . (uncurry max)) . ( id + (depth >< depth ))|
%
\just\equiv{Universal-cata}
%
...
&
     |A + (Int >< Int)|
      \ar[l]^-{\alt{\underline{1}}{succ\comp\uncurry{max}}}
}
\end{eqnarray*}

\par{ Decidimos definir as funções auxiliares depthE e depthD que calculam a profundidade da arvore desde o topo até à Leaf mais profunda do lado esquerdo / direito}
...
depth = cataLTree ( either ( const 1) (succ . umax ))

\end{code}

\par{ Definimos agora estas duas funções que calculam a que profundidade uma subarvore se encontra , facilitando assim a procura na lista }
...
\par{ Ao saber a que profundidade se encontram as subarvores a serem iteradas , para saber se temos de "escolher" a subarvore  esquerda ou direita basta verificar essa posição
na lista. Caso nessa posição da lista esteja True returna-mos 1 , caso seja Falso returna-mos 2 e caso a lista tenha menos elementos do que a arvore tem de profundidade , 
devolve-mos 0 que representa que não se vai escolher nenhum dos lados.}
...
\par{ Finalmente na função navLTree , caso a árvore recebida seja apenas uma Leaf não interessa a lista recebida pois o resultado será sempre essa mesma Leaf.
Caso seja um Fork , ao adimitir-mos que ja foi executado navLTree tanto na subarvore esquerda tanto como na direita , temos então de escolher entre o resultado da esquerda
e da direita, processo que é efetuado na função g. Função que começa por guadar em "v" o valor da buscaLista , como explicado anteriormente , caso v seja 0 , não vamos escolher nenhum lado , por isso fazemos Fork dos resultados das duas subarvores , caso v seja 1 escolhemos o resultado da subarvore esquerda e caso v sej a 2 escolhemos a direita.   }

\begin{code}


navLTree :: LTree a -> ([Bool] -> LTree a)
navLTree a l = cataLTree (either Leaf (g a l)) a
...
&
    |A + (LTree A >< LTree A)|
           \ar[d]^{|id + (cataLTree bnavLTree)|}
           \ar[l]_-{|inLTree|}
\\
     |BTree B -> LTree A|
&
     |A + (BTree B -> LTree A) >< (BTree B -> LTree A)|
           \ar[l]^-{|g|}
}
\end{eqnarray*}

A função bnavLTree foi obtida com base no diagrama, supra citado, em que o Ltree é desconstruído usando o out,
ficando um either em que o A é do tipo LTree e (LTree A >< LTree A) é a parte recursiva da função bnavLTree.
Como o funtor de Ltree é \emph{id + (cataLTree bnavLTree)}, obtemos A + (BTree B -> LTree A) >< (BTree B -> LTree A),
em que a primeira componente é do mesmo tipo de que a de cima e a segunda componente, a parte recursiva, é do tipo exponencial,
em que se aplica o tipo BTree B ao tipo Ltree A (BTree B -> LTree A) >< (BTree B -> LTree A).
Concluímos assim que bnavLTree é uma exponenciação, com um cataLTree definido em cima.
...
&
    |A + (LTree A >< LTree A)|
           \ar[d]^{|id + (cataLTree pbnavLTree)|}
           \ar[l]_-{|inLTree|}
\\
     |BTree (D B) -> Dist (LTree A)|
&
     |A + (BTree (Dist B) -> Dist (LTree A)) >< (BTree (Dist B) -> Dist (LTree A))|
           \ar[l]^-{|g|}
}
\end{eqnarray*}

Nesta função, adotamos parte da função bnavLTree e, 
usamos a maquinaria da monadificação, 
para fazermos as devidas alterações para que a função funcionasse.

Na primeira parte do either (g1), o passo base na forma de Dist (LTree A), 
conjungamos a definição de LTree com a definição do Dist.

Na segunda parte do either (g2), tivemos que monadificar tanto o passo base de g2
como o passo recursivo. Sendo que no passo base vimos que tanto o \emph{(a Empty)} como o \emph{(b Empty)}
eram ambas probabilidades e chegamos à conclusão que deveríamos juntar estas probabilidades com a função \emph{prod}, monadificar, isto é,
retirar o valor (a,b) da caixa monádica do resultado do \emph{prod} e,
posteriormente, fazer o \emph{Fork} ao passo monadificado, de maneira a obter um Dist(LTree a). 
Se não monadificassemos depois de fazer a função \emph{prod}, iriamos obter Dist (a,b) mas, não era o que queríamos pois,
queriamos que de acorde com o tipo da função pbnavLTree, isto é, Dist (LTree A).
...
main :: IO() 
main = do
  mosaico <- randomMosaico 10 10 (div 10 2)
  display janela white mosaico
\end{code}

\par{ Função randomMosaico gera um mosaico com l linhas, c colunas. Caso o numero de linhas seja 0 retorna uma imagem sem nada, caso contrario gera uma imagem de uma linha e de um ramdomMosaico com com l-1 e e retorna a sua união. }
...
randomMosaico l c al= do
  ...
  a<-randomLMosaico c (div c 2)
  let p=Pictures(( put (0,fromIntegral(80*(l-al-1))) a):[m])
  return p

\end{code}

\par{ Função randomLMosaico gera uma imagem com c colunas. Caso n numero de colunas a gerar seja 0 retorna uma imagem sem nada, caso contrario gera uma imagem um ladrinho com RandomTruchet e uma imagem com as outras colunas e retorna a imagem com a sua união. }
...
\par{ Função randomTrunchet que retorna uma imagem de um ladrinho.}
...
\end{code}
    ...
    \begin{figure}\centering
    \includegraphics[scale=0.20]{images/Mosaico.png}
    \caption{Mosaico obtido com a resuloção deste problema.}
    \label{fig:Mosaico}
    \end{figure}
